...
@section('content')
    ...
    <div class="card overflow-hidden">
        <h2 class="border-b border-slate-200 px-5 py-3.5 text-sm font-semibold text-slate-900">Order history</h2>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($orders as $order)
                            <tr class="transition hover:bg-slate-50 {{ $order->trashed() ? 'opacity-55' : '' }}">
                                ...
                                <td class="px-5 py-3 text-right text-slate-500">{{ $order->placed_at->format('d M Y, g:i A') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
