    ...
    <div x-data="productForm">
                        ...
                        <tbody class="divide-y divide-line">
                            @foreach ($products as $product)
                                ...
                                <tr class="transition hover:bg-raised">
                                    <td class="px-5 py-3 font-medium whitespace-nowrap text-ink">
                                        <a href="{{ route('products.show', $product) }}" class="hover:text-brand-700 hover:underline">
                                            {{ $product->name }}
                                        </a>
                                    </td>
                                    <td class="px-3 py-3 font-mono text-xs text-faint">{{ $product->code }}</td>
                                    <td class="tnum px-3 py-3 text-right text-body">₹{{ number_format((float) $product->unit_price, 2) }}</td>
                                    <td class="tnum px-3 py-3 text-right text-muted">{{ rtrim(rtrim(number_format((float) $product->tax_percentage, 2), '0'), '.') }}%</td>
                                    <td class="tnum px-3 py-3 text-right text-muted">
                                        {{ $product->effectiveLowStockThreshold() }}
                                        @unless ($product->low_stock_threshold)
                                            <span class="ml-0.5 text-xs text-faint" title="Falls back to the application default">def</span>
                                        @endunless
                                    </td>
            ...
            <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <x-field label="Code" for="product-code" :required="true" hint="unique">
                    ...
                    <template x-if="errors.code">
                        <p class="mt-1.5 text-xs text-danger" x-text="errors.code"></p>
                    </template>
                </x-field>
                ...
                <x-field label="Unit price" for="product-price" :required="true">
                    ...
                    <template x-if="errors.unit_price">
                        <p class="mt-1.5 text-xs text-danger" x-text="errors.unit_price"></p>
                    </template>
                </x-field>
                ...
                <x-field label="Tax rate" for="product-tax" :required="true">
                    <div class="relative">
                        ...
                        <span class="pointer-events-none absolute inset-y-0 right-3 flex items-center text-sm text-faint">%</span>
                    </div>
                    <template x-if="errors.tax_percentage">
                        <p class="mt-1.5 text-xs text-danger" x-text="errors.tax_percentage"></p>
                    </template>
                </x-field>
                ...
                <x-field label="Low stock threshold" for="product-threshold"
                    ...
                    <template x-if="errors.low_stock_threshold">
                        <p class="mt-1.5 text-xs text-danger" x-text="errors.low_stock_threshold"></p>
                    </template>
                </x-field>
                ...
                <x-field label="Opening stock" for="product-stock" :required="true" x-show="!editing">
                    ...
                    <template x-if="errors.stock_on_hand">
                        <p class="mt-1.5 text-xs text-danger" x-text="errors.stock_on_hand"></p>
                    </template>
                </x-field>

                <div class="rounded-lg bg-raised p-3 text-xs text-muted sm:col-span-2" x-show="editing">
        ...
        <x-modal title="Add stock" show="restocking" width="max-w-sm">
            <p class="mt-1 text-sm text-muted">
                ...
                <span class="font-medium text-body" x-text="editing?.name"></span>.
            </p>
            ...
            <div class="mt-4 space-y-4">
                <x-field label="Units" for="restock-quantity" :required="true">
                    ...
                    <template x-if="errors.quantity">
                        <p class="mt-1.5 text-xs text-danger" x-text="errors.quantity"></p>
                    </template>
                </x-field>
        ...
        <x-modal title="Delete this product?" show="confirming" width="max-w-sm">
            <p class="mt-1 text-sm text-muted">
                <span class="font-medium text-body" x-text="editing?.name"></span> comes out of the catalogue
